...
def demo_spike_detection():
    ...
    viral_signals = detector.detect_viral_keywords(
        source=None,
        window_days=3,
        min_growth_rate=2.0
    )
    ...
    if viral_signals:
        ...
        for i, signal in enumerate(viral_signals[:10], 1):
            ...
            print(f"{i}. {signal.keyword} ({signal.source})")
            print(f"   📊 초기: {signal.baseline_value:.1f} → "
                  f"최근: {signal.current_value:.1f}")
            print(f"   🚀 성장률: {signal.spike_ratio:.2f}x "
                  f"(일일 +{growth_per_day*100:.1f}%)")
            print(f"   ⭐ 점수: {signal.spike_score:.1f}/100")
            print()
    else:
        print("바이럴 키워드가 없습니다.\n")
    ...
    for spike_type, signals in all_spikes.items():
        ...
        for i, signal in enumerate(signals, 1):
            print(f"  {i}. {signal.keyword} ({signal.spike_ratio:.2f}x, "
