...
def test_snapshot_database_creates_file(tmp_path: Path) -> None:
    from storage.date_storage import snapshot_database
...
def test_cleanup_date_directories_removes_old(tmp_path: Path) -> None:
    from storage.date_storage import cleanup_date_directories
...
def test_cleanup_dated_reports(tmp_path: Path) -> None:
    from storage.date_storage import cleanup_dated_reports
...
def test_storage_create_daily_snapshot(tmp_path: Path) -> None:
    from storage.trend_store import create_daily_snapshot

    db_path = tmp_path / "data" / "trendradar.duckdb"
    db_path.parent.mkdir(parents=True, exist_ok=True)
    db_path.write_text("fake-db")

    result = create_daily_snapshot(db_path=db_path)
...
def test_storage_create_daily_snapshot_custom_dir(tmp_path: Path) -> None:
    ...
    result = create_daily_snapshot(db_path=db_path, snapshot_dir=custom_dir)
...
def test_storage_cleanup_old_snapshots(tmp_path: Path) -> None:
    from storage.trend_store import cleanup_old_snapshots
    ...
    removed = cleanup_old_snapshots(db_path=db_path, keep_days=30)
    assert removed == 1
    assert not old_dir.exists()
